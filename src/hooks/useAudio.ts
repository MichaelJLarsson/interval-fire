...
import { type AudioSource, createAudioPlayer, setAudioModeAsync } from 'expo-audio'
...
export function useAudio() {
  ...
  useEffect(() => {
    // Without this, iOS silences all app audio when the device's ring/silent
    // switch is set to silent — the simulator has no such switch, so cues
    // play there regardless and the bug only shows up on a real device.
  ...
  const speak = useCallback((phrase: VoicePhrase) => {
    ...
    voicePlayerRef.current?.remove()
    const player = createAudioPlayer(VOICE_ASSETS[phrase] as AudioSource)
    player.play()
